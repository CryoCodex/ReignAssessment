...
class Toast {
    ...
    @discardableResult
    init(duration: Double = 0, text: String, container: UINavigationController? = nil, backgroundColor: UIColor = .yellow, direction: ToastDirection = .bottom, completion: (() -> Void)? = nil) {
        let screenSize = UIScreen.main.bounds
        let labelHeight: CGFloat = 66
        let rect = CGRect(
            x: 12,
            y: direction == .top ? -labelHeight : UIScreen.main.bounds.height + labelHeight,
